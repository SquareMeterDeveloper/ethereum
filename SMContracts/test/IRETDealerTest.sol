...
contract IRETDealerTest is ContractReceiver {
    ...
    function test1() public {
        ...
        cny.approve(store, 6000000);
        store.purchase(10000);
        Assert.equal(cny.balanceOf(this), 185000000, "余额应为185000000");
        Assert.equal(store.getPurchased(this), 25000, "已购买份数不是25000");
        Assert.equal(store.getTotalLeft(), 5000, "剩余可购份数不是5000");
        Assert.equal(store.getUserLeft(this), 0, "剩余可购份数不是0");
